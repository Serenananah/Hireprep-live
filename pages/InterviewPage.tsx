...
import Avatar from '../components/InterviewAvatar';
      ...
      <div className="w-full lg:w-[70%] flex flex-col gap-4 min-w-0">
        ...
        <div className="relative h-[300px] md:h-[350px] rounded-3xl overflow-hidden shadow-xl bg-black border border-white/10 flex-none group">
          <div className="absolute inset-0 bg-gradient-to-br from-indigo-900 to-slate-900">
             ...
             <div className={`absolute -inset-48 bg-cyan-500/10 rounded-full blur-[120px] transition-opacity duration-1000 ${isConnected ? 'opacity-100' : 'opacity-0'}`} />
          </div>
          
          <div className="absolute inset-0 flex flex-col items-center justify-center z-10">
            {/* The Interactive Avatar */}
            <div className={`transform transition-all duration-700 ${isConnected ? 'scale-100 opacity-100' : 'scale-90 opacity-50 grayscale'}`}>
               <Avatar 
                  currentStep={state.avatarBehavior} 
                  isSpeaking={state.isAiSpeaking} 
               />
            </div>

            <div className="absolute top-6 right-6">
                <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-mono font-bold border backdrop-blur-md transition-all ${isConnected ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-400' : 'bg-red-500/10 border-red-500/30 text-red-400'}`}>
                    <Wifi className="w-3 h-3" />
                    {isConnected ? "LIVE CONNECTION" : "OFFLINE"}
                </div>
            </div>

            {/* Telemetry / Behavior System */}
            <div className="absolute bottom-6 left-6 flex flex-col gap-2 bg-black/40 backdrop-blur-2xl border border-white/10 p-4 rounded-2xl min-w-[160px] shadow-2xl z-20">
               <div className="flex items-center gap-2 text-[9px] uppercase font-black text-amber-500 tracking-[0.2em] mb-2 border-b border-white/5 pb-2">
                   <Activity size={12} />
                   Behavior System
               </div>
               <div className="space-y-3">
                   <div className="space-y-1">
                      <div className="text-[8px] text-slate-500 uppercase font-black tracking-tighter">Current Expression</div>
                      <div className="text-xs text-white font-mono bg-white/5 px-2 py-1 rounded border border-white/5 capitalize truncate">
                        {state.avatarBehavior.facial_expression.replace(/_/g, ' ')}
                      </div>
                   </div>
                   <div className="space-y-1">
                      <div className="text-[8px] text-slate-500 uppercase font-black tracking-tighter">Kinetic State</div>
                      <div className="text-xs text-white font-mono bg-white/5 px-2 py-1 rounded border border-white/5 capitalize truncate">
                        {state.avatarBehavior.head_movement.replace(/_/g, ' ')}
                      </div>
                   </div>
               </div>
            </div>
            
            {/* Connection Status Text (only when not connected) */}
            {!isConnected && (
              <div className="absolute bottom-10 text-center">
                 <h2 className="text-xl font-bold text-white/50 tracking-widest">AI AGENT OFFLINE</h2>
              </div>
            )}
          </div>
        </div>
