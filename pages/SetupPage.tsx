...
import { fileService, ParsedDocument } from '../services/pdf/fileService';
...
const SetupPage: React.FC<SetupPageProps> = ({ user, onStartInterview }) => {
  ...
  const handleStart = async () => {
    ...
    try {
      ...
      // Fix: Ensure userId is a string to match service definition
      await fileService.initializeRagSession(String(user?.id || 'guest'), jdDoc, resumeDoc);
  ...
  return (
    <div className="w-full max-w-7xl mx-auto space-y-6 animate-fade-in pb-4">
      ...
      {/* Header */}
      <div className="relative z-10">     {/* ★★★ 关键 */}
